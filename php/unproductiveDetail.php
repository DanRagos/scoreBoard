 	...
 	if (isset($_POST['start']) && isset($_POST['end'])){
		...
		if ($start == 'none' && $end == 'none') {
			...
			if ($isToday == '1') {
				...
				$query = "call gemba_db.getDeviceProductionDowntimeByDateFilterCompleted('" . $start . "','" . $end . "'," . $_POST['id'] . ")";
			}
		}
		...
		if ($result) { // if (mysqli_num_rows($result) > 0) {
			while($row = mysqli_fetch_array($result)) {
				...
				$summary = array(
					...
					'devend' => ($row["realEndTime"] == null ? "--" : $row["realEndTime"]),  
					'devdur' => $duration,
					'devrem' => $row['remarks'],
					'devucs' => ($row['unproductive_cause'] == null ? '--' : $row['unproductive_cause']),
					'rowCnt' => $rowcnt,
